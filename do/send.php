...
$queryarray = query("SELECT * FROM participants_do WHERE status = 'valid'");
confirm($queryarray);
...
$update_status = query("UPDATE participants_do SET status = 'invalid' WHERE id = '{$winner}'");
...
$insert_winner = query("INSERT INTO winners(win_name, win_level, win_school, win_position) VALUES('{$namearray[$id][0]}','DO','{$namearray[$id][2]}','{$namearray[$id][3]}')");
